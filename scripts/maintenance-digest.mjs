...
const marker = "<!-- agent-secret-guard-maintenance-digest -->";
const now = new Date();
...
async function api(path, options = {}) {
  const response = await fetch(`${apiBase}${path}`, {
    ...
    headers: {
      ...
      "User-Agent": "agent-secret-guard-maintenance",
      ...(options.headers ?? {})
    }
  });
...
async function main() {
  ...
  const failedDefaultRuns = runs.workflow_runs.filter((run) =>
  ...
  const openDependabotAlerts = dependabotAlerts.ok ? dependabotAlerts.data : [];
  const openCodeScanningAlerts = codeScanningAlerts.ok ? codeScanningAlerts.data : [];

  const hasProblems =
    stalePulls.length > 0 ||
    openBugs.length > 0 ||
    failedDefaultRuns.length > 0 ||
    openDependabotAlerts.length > 0 ||
    openCodeScanningAlerts.length > 0;
  ...
  const body = [
    ...
    linkList(failedDefaultRuns, (run) => `- ${run.name}: ${run.conclusion} (${run.html_url})`),
    "",
    "### Dependabot Alerts",
    dependabotAlerts.ok
      ? linkList(openDependabotAlerts, (alert) => `- ${alert.dependency?.package?.name ?? "dependency"} ${alert.security_advisory?.severity ?? "unknown"} (${alert.html_url})`)
      : `- Not readable by this workflow token: ${dependabotAlerts.message}`,
    "",
    "### Code Scanning Alerts",
    codeScanningAlerts.ok
      ? linkList(openCodeScanningAlerts, (alert) => `- ${alert.rule?.id ?? "rule"} ${alert.most_recent_instance?.message?.text ?? ""} (${alert.html_url})`)
      : `- Not readable by this workflow token: ${codeScanningAlerts.message}`,
    "",
    "## Expected Response",
    "",
    "- Fix failed default-branch or scheduled workflows first.",
    "- Review security alerts privately; do not paste secrets or sensitive paths into public comments.",
    "- Assign well-scoped implementation work to a coding agent only after the issue contains clear acceptance checks."
  ].join("\n");
